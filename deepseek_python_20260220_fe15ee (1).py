...
def load_data():
    ...
    global user_achievements, daily_bonus, jackpot, duels, logs, user_quests, event_data
    global user_cases
    ...
    daily_bonus = safe_json_load('daily_bonus.json', {})
...
def save_data():
    ...
    with open('daily_bonus.json', 'w', encoding='utf-8') as f:
        json.dump(daily_bonus, f, ensure_ascii=False, indent=2)
...
def start_gold_game(message, bet, level):
    user_id = str(message.from_user.id)
    user = get_user(user_id)
    if bet > user['balance']:
        bot.send_message(message.chat.id, f"❌ Недостаточно средств. Твой баланс: {user['balance']}.")
    ...
    if bet > MAX_BET:
        ...
        return
    user['balance'] -= bet
    update_quest_progress(user_id, 'play', 1, 'gold')
    success_chance = 1.0 / (level + 1)
    if random.random() < success_chance:
        mult = GOLD_MULTIPLIERS[level]
        ...
        update_quest_progress(user_id, 'win', 1)
        update_quest_progress(user_id, 'earn', win // 1000)
        update_event_stats(user_id, 'gold', win)
        result = f"🥇 ЗОЛОТО! Уровень {level}\n💰 Выигрыш: {win} (x{mult})"
    else:
        result = f"❌ Неудача! Уровень {level}\n💰 Проигрыш: {bet}"
        user['total_losses'] += 1
        user['win_streak'] = 0
        user['total_lost'] += bet
        update_event_stats(user_id, 'gold', 0)
    user['max_win_streak'] = max(user['max_win_streak'], user['win_streak'])
    save_data()
    result += f"\n💰 Баланс: {user['balance']}"
...
def start_diamond_game(message, bet, level):
    user_id = str(message.from_user.id)
    user = get_user(user_id)
    if bet > user['balance']:
        bot.send_message(message.chat.id, f"❌ Недостаточно средств. Твой баланс: {user['balance']}.")
    ...
    if bet > MAX_BET:
        ...
        return
    user['balance'] -= bet
    update_quest_progress(user_id, 'play', 1, 'diamond')
    success_chance = 0.9 - (level * 0.05)
    if random.random() < success_chance:
        mult = DIAMOND_MULTIPLIERS[level]
        win = int(bet * mult * get_global_multiplier(user_id) * get_event_multiplier())
        user['balance'] += win
        user['total_wins'] += 1
        user['win_streak'] += 1
        update_quest_progress(user_id, 'win', 1)
        update_quest_progress(user_id, 'earn', win // 1000)
        update_event_stats(user_id, 'diamond', win)
        result = f"💎 АЛМАЗ! Уровень {level}\n💰 Выигрыш: {win} (x{mult})"
    else:
        result = f"❌ Неудача! Уровень {level}\n💰 Проигрыш: {bet}"
        user['total_losses'] += 1
        user['win_streak'] = 0
        user['total_lost'] += bet
        update_event_stats(user_id, 'diamond', 0)
    user['max_win_streak'] = max(user['max_win_streak'], user['win_streak'])
    save_data()
    result += f"\n💰 Баланс: {user['balance']}"
    bot.send_message(message.chat.id, result)
...
def open_case(user_id, case_id):
    ...
    if case_id == 'case6':
        ...
        # Здесь можно добавить проверку права на открытие ивентового кейса
...
def claim_daily_bonus(user_id):
    user = get_user(user_id)
    now = time.time()
    if user_id not in daily_bonus:
        daily_bonus[user_id] = {'last_claim': 0, 'streak': 0}
    last = daily_bonus[user_id]['last_claim']
    ...
    if RELEASE_EVENT['active'] and time.time() < RELEASE_EVENT['end_time']:
        bonus = int(bonus * RELEASE_EVENT['multiplier'])
    bonus = int(bonus * get_global_multiplier(user_id))
    user['balance'] += bonus
    daily_bonus[user_id] = {'last_claim': now, 'streak': streak}
    save_data()
...
def start_russian_roulette_game(message, bet):
    ...
    if random.random() < survival_chance:
        win = int(bet * 2.135 * get_global_multiplier(user_id) * get_event_multiplier())
        user['balance'] += win
        user['total_wins'] = user.get('total_wins', 0) + 1
        user['win_streak'] = user.get('win_streak', 0) + 1
        update_quest_progress(user_id, 'win', 1)
        ...
        result_text = f"😌 Щелчок... Ты выжил!\n💰 Ты выиграл {win} кредиксов (x2.135)!"
    else:
...
def show_profile(chat_id, user_id):
    ...
    text += f"👤 ID: {user_id}\n"
    text += f"💸 Ваш баланс: {user['balance']} кредиксов 💫\n"
...
def get_games_text():
    games_text = (
        ...
        "🥇 Золото - Копай золото\n"
        "   Команда: золото [ставка] [уровень] (уровни 1-5)\n\n"
        
        "📈 Хило - Рискни!\n"
        "   Команда: хило [ставка] [low/medium/high]\n\n"
        
        "💎 Алмазы - Ищи алмазы\n"
        "   Команда: алмазы [ставка] [уровень] (уровни 1-10)\n\n"
        
        "🏀 Баскетбол - Угадай, попадёт или нет\n"
        "   Команда: баскетбол [ставка] [гол/мимо]\n\n"
        
        "🔺 Пирамида - 4 ячейки, одна мина\n"
        ...
        "👥 Клан - Клановая система\n"
        "   Команда: /clan\n\n"
        
        "⚔️ Дуэли - Сражения с друзьями\n"
        "   Команда: /duel @username ставка игра\n\n"
        
        "💼 Бизнес - Купить и улучшать бизнес\n"
        "   Команда: /business\n\n"
        
        "🏦 Банк - Положить/снять кредиксы\n"
        "   Кнопка: 🏦 Банк\n\n"
        
        "🦫 Маркет - Купить бобров\n"
        "   Кнопка: 🦫 Маркет или команда /market (скоро)\n\n"
        
        "📦 Кейсы - Открывай кейсы\n"
        "   Кнопка: 📦 Кейсы или команда /кейсы\n\n"
        
        "🎉 Ивент - Информация о текущем ивенте\n"
        "   Кнопка: 🎉 Ивент\n\n"
        ...
        "💰 Перевод валюты\n"
        "   Команда: /дать @username сумма\n\n"
        
        "🛑 Отмена игры\n"
        "   Команда: /cancel\n\n"
        
        "❓ Помощь\n"
        "   Команда: /help или /games\n\n"
...
def handle_all_messages(message):
    ...
    if user.get('game'):
        if user['game'].get('stage') == 'waiting_bet':
            try:
                bet = parse_bet(text)
                if bet is None:
                    return
                ...
                if game_type == 'tower':
                    ...
                    level = user['game'].get('level', 1)
                    start_gold_game(message, bet, level)
                elif game_type == 'hilo':
                    risk = text.lower()
                    ...
                    level = user['game'].get('level', 1)
                    start_diamond_game(message, bet, level)
                    ...
                    bet_type = user['game'].get('roulette_type')
                    bet_value = user['game'].get('roulette_value')
                    start_roulette_game(message, bet, bet_type, bet_value)
            except Exception as e:
                bot.send_message(message.chat.id, f"❌ Ошибка: {e}")
        elif user['game'].get('type') == 'bank' and user['game'].get('stage') in ['deposit', 'withdraw']:
            try:
    ...
    elif lower_text.startswith('золото '):
        try:
            parts = text.split()
            if len(parts) != 3:
                bot.send_message(message.chat.id, "❌ Использование: золото [ставка] [уровень]")
                return
            bet = parse_bet(parts[1])
            level = int(parts[2])
            if bet is None or level < 1 or level > 5:
                bot.send_message(message.chat.id, "❌ Неверная ставка или уровень (1-5).")
                return
            start_gold_game(message, bet, level)
        except:
            bot.send_message(message.chat.id, "❌ Пример: золото 1000 3")
    ...
    elif lower_text.startswith('алмазы '):
        try:
            parts = text.split()
            if len(parts) != 3:
                bot.send_message(message.chat.id, "❌ Использование: алмазы [ставка] [уровень]")
                return
            bet = parse_bet(parts[1])
            level = int(parts[2])
            if bet is None or level < 1 or level > 10:
                bot.send_message(message.chat.id, "❌ Неверная ставка или уровень (1-10).")
                return
            start_diamond_game(message, bet, level)
        except:
            bot.send_message(message.chat.id, "❌ Пример: алмазы 1000 5")
...
def process_menu_button(message, button_text):
    user_id = str(message.from_user.id)
    user = get_user(user_id)
    if user['balance'] <= 0 and button_text not in ['💰 Баланс', '👥 Рефералы', '🏆 Топ', '💎 Бонус', '🏆 Достижения', '📱 Профиль', '📋 Задания', '🎉 Ивент', '📦 Кейсы']:
    ...
    if button_text == '🏰 Башня':
        ...
        save_data()
        set_game_timer(user_id)
    elif button_text == '🥇 Золото':
        bot.send_message(message.chat.id, 
                        "🥇 Введи ставку и уровень (1-5) через пробел:\n"
                        "Например: `1000 3`\n"
                        "🚫 Для отмены введите /cancel")
        user['game'] = {'type': 'gold', 'stage': 'waiting_bet'}
        save_data()
        set_game_timer(user_id)
        ...
        save_data()
        set_game_timer(user_id)
    elif button_text == '💎 Алмазы':
        bot.send_message(message.chat.id, 
                        "💎 Введи ставку и уровень (1-10) через пробел:\n"
                        "Например: `1000 5`\n"
                        "🚫 Для отмены введите /cancel")
        user['game'] = {'type': 'diamond', 'stage': 'waiting_bet'}
        save_data()
        set_game_timer(user_id)
        ...
        show_ref_info(user_id, message.chat.id)
    elif button_text == '🏆 Топ':
        show_top(message.chat.id)
    elif button_text == '🏦 Банк':
        apply_bank_interest(user_id)
        show_bank_menu(message.chat.id, user_id)
    elif button_text == '🦫 Маркет':
        show_market_menu(message.chat.id, user_id)
    elif button_text == '💼 Бизнес':
        show_business_menu(message.chat.id, user_id)
    elif button_text == '👥 Клан':
        show_clan_menu(message.chat.id, user_id)
    elif button_text == '⚔️ Дуэли':
        bot.send_message(message.chat.id, 
                        "⚔️ Дуэли\n\n"
                        "Создать дуэль: /duel @username ставка игра\n"
                        "Игры: tower, mines, jackpot, crash, blackjack, roulette")
    elif button_text == '🏆 Достижения':
        ...
        success, msg = claim_daily_bonus(user_id)
        bot.send_message(message.chat.id, msg)
    elif button_text == '📋 Задания':
        show_quests(message.chat.id, user_id)
    elif button_text == '🎉 Ивент':
...
def callback_handler(call):
    ...
    # Золото
        ...
        level = int(parts[1])
        bet = int(parts[2])
        start_gold_game(call.message, bet, level)
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
    # Хило
    ...
    # Алмазы
        ...
        level = int(parts[1])
        bet = int(parts[2])
        start_diamond_game(call.message, bet, level)
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
    # Баскетбол
    ...
    # Рулетка
        ...
        if game and game['type'] == 'roulette' and game.get('stage') == 'choosing_roulette_number':
            ...
            if win > 0:
                ...
                update_quest_progress(user_id, 'earn', win // 1000)
                update_event_stats(user_id, 'roulette', win)
                result_text = f"🎉 Выпало {number} {get_color_emoji(color)}! Ты угадал число {chosen_number}!\n💰 Ты выиграл {win} кредиксов (x{mult})"
            else:
                result_text = f"❌ Выпало {number} {get_color_emoji(color)}. Ты не угадал число {chosen_number}.\n💰 Ты проиграл {bet} кредиксов."
                user['total_losses'] = user.get('total_losses', 0) + 1
                user['win_streak'] = 0
                user['total_lost'] = user.get('total_lost', 0) + bet
                update_event_stats(user_id, 'roulette', 0)
    ...
    # Кланы
        ...
        clan_name = user.get('clan')
        if clan_name and clan_name in clans:
            show_clan_info(call.message.chat.id, clan_name)
        bot.answer_callback_query(call.id)
    elif data == 'clan_leave':
        leave_clan(call.message, user_id)
        bot.answer_callback_query(call.id)
    elif data == 'clan_refresh':
        show_clan_menu(call.message.chat.id, user_id)
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
    # Банк
    elif data.startswith('bank_'):
        bank_handlers(call, user_id, user)
    # Маркет
    elif data.startswith('market_'):
        market_handlers(call, user_id, user)

def bank_handlers(call, user_id, user):
    data = call.data
    bank = user.get('bank', {'balance': 0, 'history': []})
    if data == 'bank_balance':
        ...
        save_data()
        bot.edit_message_text("📥 Введите сумму для пополнения (можно использовать к и кк, например: 100к, 5кк):", 
                             call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
    elif data == 'bank_withdraw':
        user['game'] = {'type': 'bank', 'stage': 'withdraw'}
        save_data()
        bot.edit_message_text("📤 Введите сумму для снятия (можно использовать к и кк, например: 100к, 5кк):", 
                             call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
...
def market_handlers(call, user_id, user):
    data = call.data
    if data == 'market_collection':
        ...
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
    elif data == 'market_exit':
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
    elif data.startswith('market_buy_'):
        beaver_id = data.replace('market_buy_', '')
        success, message_text = buy_beaver(user_id, beaver_id)
...
if __name__ == '__main__':
    ...
    bg_thread.start()
    print("🚀 Бот запущен!")
    print("✅ Валюта: кредиксы")
    print("✅ Добавлены новые игры: Башня, Футбол, Баскетбол, Пирамида")
    print("✅ Добавлена система кейсов (6 видов)")
    print("✅ Обновлён маркет бобров")
    print("✅ Добавлена автоматическая отмена игры")
    print("✅ Команда /дать работает")
    print("✅ Команда /кейсы добавлена")
    print("✅ Список помощи обновлён")
    print(f"👥 Загружено пользователей: {len(users)}")
    print(f"👥 Загружено кланов: {len(clans)}")
    print(f"💼 Загружено бизнесов: {len(businesses)}")
    print(f"🎉 Ивент активен: {'✅' if RELEASE_EVENT['active'] else '❌'}")
